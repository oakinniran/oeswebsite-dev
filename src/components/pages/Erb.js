...
function Erb() {
    ...
    async function getServerLecturesProps(){
        const api_urls = 'https://applications.oes.com.ng/OESWebApp/getallerbschoollecture.do';
        ...
        erdata = data.map(function(course){
            return(
                <ECourse src={course.subjectvideo} course={course.subject} title={course.subjectTitle} author={course.teacher} duration="1 Week" Level={course.classLevel} description="test" hashtag={course.teacher} cost={course.cost}  link={course.courseGroupTab}   logo={course.logo} picture={course.picture} /> 
            )
        })
